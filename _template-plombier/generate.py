#!/usr/bin/env python3
"""
generate.py — Générateur de site plombier
...
for z in zones:
    zone_chips_html += (
        f'<a href="/plombier-{z["SLUG"]}/" class="zone-chip">'
...
for z in zones:
    footer_zone_links_html += (
        f'<li><a href="/plombier-{z["SLUG"]}/">Plombier à {z["VILLE"]}</a></li>\n'
    )
...
def photo_tag(src, alt="Réalisation plomberie"):
    if src:
...
def _alt(i):
    return photo_alts[i-1] if i <= len(photo_alts) and photo_alts[i-1] else f"Réalisation plomberie {i}"
...
for zone in zones:
    ...
    z_title_long  = f"Plombier à {z_ville} — {z_ent} | Devis gratuit"
    z_title_short = f"Plombier à {z_ville} — {z_ent}"
    ...
    write_page(OUTPUT / f"plombier-{zone['SLUG']}", zone_html)
    print(f"✓ plombier-{zone['SLUG']}/index.html")
...
for zone in zones:
    sitemap_urls.append(f"https://{domain}/plombier-{zone['SLUG']}/")
sitemap_urls += [
